...
func (i *Install) Run(chrt *chart.Chart, vals map[string]interface{}) (*release.Release, error) {
	...
	appLabels, err := ha.GetAppLabels(rel, i.cfg)
	if err != nil {
		return nil, err
	}
	err = resources.Visit(ha.SetMetadataVisitor(rel.Name, rel.Namespace, appLabels, true))
	if err != nil {
		return nil, err
	}
	...
	if !i.ClientOnly && !isUpgrade && len(resources) > 0 {
		toBeAdopted, err = ha.ExistingResourceConflict(resources, rel.Name, rel.Namespace)
		if err != nil {
...
func (i *Install) failRelease(rel *release.Release, err error) (*release.Release, error) {
	...
	if i.Atomic {
		...
		uninstall := ha.NewUninstall(i.cfg)
